...
function custom_attribute_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_add({$options}) called", 3);
...
EOM
    ...
    if (!$host['id'] and !$subnet['id'] and !$vlan['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => No host, subnet or vlan found!",3);
    ...
    if (!$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find custom attribute type: {$options['type']}",3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The type '{$catype['name']}' is already in use on {$desc}",3);
    ...
    if ($catype['field_validation_rule'] and !preg_match($catype['field_validation_rule'], $options['value'])) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The value '{$options['value']}' does not match field validation rule: {$catype['field_validation_rule']}",3);
...
function custom_attribute_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_del({$options}) called", 3);
...
EOM
    ...
    if (is_numeric($options['type'])) {
        ...
        if (!$catype['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find custom attribute type using the ID {$options['name']}!",3);
        ...
        if (!$catype['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find custom attribute type using the name {$options['type']}!",3);
    ...
    list($status, $rows, $record) = ona_get_custom_attribute_record(array('table_name_ref' => $table_name_ref, 'table_id_ref' => $table_id_ref, 'custom_attribute_type_id' => $catype['id']));
    if (!$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find custom attribute!",3);
...
function custom_attribute_modify($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_modify({$options}) called", 3);
...
EOM
    ...
    if ($status or !$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid Custom Atribute record ID ({$options['id']})!",3);
        $self['error'] = "ERROR => Invalid Custom Atribute record ID ({$options['id']})!";
        return(array(2, $self['error']. "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_modify(): Found entry, {$entry['name']} => {$entry['value']}", 3);
    $desc='';
    ...
    if (!$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find custom attribute type: {$typeval}",3);
    ...
    if ($catype['field_validation_rule'] and !preg_match($catype['field_validation_rule'], $SET['value'])) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The value '{$SET['value']}' does not match field validation rule: {$catype['field_validation_rule']}",3);
...
function custom_attribute_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_display({$options}) called", 3);
...
function custom_attribute_type_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => custom_attribute_type_display({$options}) called", 3);
