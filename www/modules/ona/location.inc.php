...
function location_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => location_add({$options}) called", 3);
...
EOM
    ...
    if ($status or $rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The location {$options['reference']} already exists!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new location: $id", 3);
...
function location_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => location_del({$options}) called", 3);
...
EOM
    ...
    if ($status or !$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The location specified, {$options['reference']}, does not exist!", 3);
    ...
    if ($rows != 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The location ({$loc['reference']}) is in use by {$rows} devices(s)!",3);
...
function location_modify($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => location_modify({$options}) called", 3);
...
EOM
    ...
    if (!$loc['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find location using: {$options['reference']}!",3);
        $self['error'] = "ERROR => Unable to find location using: {$options['reference']}!";
        return(array(1, $self['error'] . "\n"));
    }


    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Found location: {$loc['reference']}", 3);
