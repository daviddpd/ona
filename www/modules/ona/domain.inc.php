...
function domain_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_add({$options}) called", 3);
...
EOM
    ...
    if ($options['parent']) {
        ...
        if (!isset($parent_domain['id'])) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The parent domain specified ({$options['parent']}) does not exist!", 3);
    ...
    if ($record['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The domain specified ({$record['name']}) already exists!", 3);
    ...
    if (is_string($options['name'])) {
        ...
        if (!is_string($domain_name)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The domain name ({$options['name']}) is invalid!", 3);
...
// 
//         if (!$ohost['id']) {
//             printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The primary master host specified ({$primary}) does not exist!", 3);
...
//     }
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_add(): New domain ID: {$id} name: {$domain_name}.{$parent_domain['fqdn']}", 3);
...
function domain_del($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_del({$options}) called", 3);
...
EOM
    ...
    if (!$entry['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find a domain record using ID {$options['domain']}!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Domain selected: {$entry['name']}.{$tmp_parent['name']}", 3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Domain ({$entry['name']}) can't be deleted, it is in use by {$rows} DNS entries!",3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Domain ({$entry['name']}) can't be deleted, it is the parent of {$rows} other domain(s)!",3);
...
function domain_modify($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_modify({$options}) called", 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_modify(): Found entry, {$entry['name']}", 3);
    ...
    if (array_key_exists('set_parent',$options) and $options['set_parent']) {
        ...
        if (!$domain['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The parent domain specified ({$options['set_parent']}) does not exist!",3);
    ...
    if (is_string($options['set_name'])) {
        ...
        if ($domain['id'] and $domain['id'] != $entry['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The domain specified ({$options['set_name']}) already exists!",3);
...
/*    if ($SET['primary_master']) {
        ...
        if (!$host['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The primary master host specified ({$SET['primary_master']}) does not exist!",3);
...
function domain_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_display({$options}) called", 3);
...
EOM
    ...
    // Debugging
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_display(): Found {$domain['name']}", 3);
