...
function dhcp_entry_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_add({$options}) called", 3);
...
EOM
    ...
    if (strlen($dhcp_option_value) == 0 ) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The DHCP value was blank",3);
    ...
    if ($options['global'] == 'Y') {
        ...
        if (!$subnet['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The subnet specified, {$options['subnet']}, does not exist!",3);
        ...
        if (!$host['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server specified, {$options['server']}, does not exist!",3);
        ...
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The host specified, {$host['fqdn']}, is not a DHCP server!",3);
    ...
    if (!$type['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The DHCP parameter type specified, {$options['option']}, does not exist!",3);
        $self['error'] = "ERROR => The DHCP parameter type specified, {$options['option']}, does not exist!";
        return(array(8, $self['error'] . "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_add(): Found DHCP option {$type['display_name']}", 3);
    ...
    if ($status or $rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => That DHCP option, {$type['display_name']}, is already defined!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_add(): New ID: $id", 3);
...
function dhcp_entry_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_del({$options}) called", 3);
...
EOM
    ...
    if (is_numeric($options['id'])) {
        ...
        // Debugging
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => DHCP entry ID selected: {$options['id']}", 3);
        ...
        if (!$entry['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find the DHCP entry record using ID {$options['id']}!",3);
        ...
        if ($entry['host_id'] or $entry['server_id']) {
            ...
            if (!$host['id']) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The ID specified, {$search}, does not exist!",3);
                $self['error'] = "ERROR => The ID specified, {$search}, does not exist!";
                return(array(3, $self['error']. "\n"));
            }

            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_del(): Using host: {$host['fqdn']} ID: {$host['id']}", 3);
            $desc = $host['fqdn'];
            $lvl = $host['lvl'];
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_del(): Using subnet: {$subnet['name']} ID: {$subnet['id']}", 3);
            $desc = "{$subnet['name']} (". ip_mangle($subnet['ip_addr']).")";
            $lvl = $subnet['lvl'];
...
function dhcp_entry_modify($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_modify({$options}) called", 3);
...
EOM
    ...
    if ($status or !$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid DHCP entry record ID ({$options['id']})!",3);
        $self['error'] = "ERROR => Invalid DHCP entry record ID ({$options['id']})!";
        return(array(2, $self['error']. "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_modify(): Found entry, {$entry['display_name']} => {$entry['value']}", 3);
    $desc='';
    ...
    if ($options['set_option']) {
        ...
        if (!array_key_exists('set_value', $options)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => No value specified for given DHCP parameter type ({$options['set_option']})!", 3);
        ...
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid DHCP parameter type specified ({$options['set_option']})!", 3);
            $self['error'] = "ERROR => Invalid DHCP parameter type specified ({$options['set_option']})!";
            return(array(8, $self['error'] . "\n"));
        }

        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_modify(): Found parameter type {$type['display_name']}", 3);
        ...
        if ($status or $rows > 1 or ($rows == 1 and $record['id'] != $entry['id']) ) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => That DHCP parameter type is already defined ({$search})!", 3);
...
function dhcp_entry_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_display({$options}) called", 3);
...
EOM
    ...
    if ($options['host']) {
        ...
        if (!$server['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The host specified, {$host['FQDN']}, is not a server!", 3);
    ...
    // Debugging
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_entry_display(): Found {$anchor}: {$desc}", 3);
