...
function tag_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => tag_add({$options}) called", 3);
...
EOM
    ...
    foreach ($tags as $t) {
      if (in_array($options['name'], $t)) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The tag {$options['name']} is already associated with this {$options['type']}!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new tag: $id", 3);
...
function tag_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => tag_del({$options}) called", 3);
...
EOM
    ...
    if (is_numeric($options['tag'])) {
        ...
        if (!$tag['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find tag ({$options['tag']})!",3);
