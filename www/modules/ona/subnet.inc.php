...
function subnet_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => subnet_display('.$options.') called', 3);
...
function subnet_add($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => subnet_add('.$options.') called', 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "Subnet type selected: {$subnet_type['name']} ({$subnet_type['short_name']})", 1);
    ...
    if ($options['vlan'] or $options['campus']) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "VLAN selected: {$vlan['name']} in {$vlan['vlan_campus_name']} campus", 1);
        $SET['vlan_id'] = $vlan['id'];
    }
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new subnet: " . $id, 1);
    $SET['id'] = $id;
...
function subnet_modify($options="") {
    global $conf, $self, $onadb;
    //printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => subnet_modify('.implode (";",$options).') called', 3);
...
EOM
    ...
    if ($options['set_type']) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "Subnet type selected: {$subnet_type['display_name']} ({$subnet_type['short_name']})", 1);
        $SET['subnet_type_id'] = $subnet_type['id'];
    }
...
function subnet_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => subnet_del('.$options.') called', 3);
...
function subnet_nextip($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => subnet_nextip('.$options.') called', 3);
