...
function dhcp_server_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_add({$options}) called", 3);
...
EOM
    ...
    if (!$subnet['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find the subnet record using {$options['subnet']}!",3);
        $self['error'] = "ERROR => Unable to find the subnet record using {$options['subnet']}!";
        return(array(4, $self['error']. "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_add(): Found subnet, {$subnet['name']}", 3);

    // Determine the server is valid
    list($status, $rows, $host) = ona_find_host($options['server']);

    if (!$host['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server ({$options['server']}) does not exist!",3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Subnet {$subnet['name']} already assigned to {$host['fqdn']}",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_add(): New dhcp server subnet ID: $id", 3);
...
function dhcp_server_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_del({$options}) called", 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_del(): Found subnet, {$subnet['name']}", 3);
    ...
    //printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_server_del(): Found server, {$host['FQDN']}", 3);

    // Test that this subnet is even assigned to the server
    list($status, $rows, $dhcpserver) = ona_get_dhcp_server_subnet_record(array('host_id' => $host['id'],'subnet_id' => $subnet['id']));
    if (!$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find {$subnet['name']} on server {$host['fqdn']}",3);
    ...
    if ($options['commit'] == 'Y') {
        ...
        foreach($pools as $pool) {
            if ($pool['dhcp_failover_group_id']) {
                ...
                if ($foundfg > 0) {
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Subnet ({$subnet['name']}) has a pool assigned to this Server ({$host['fqdn']}), which is part of a failover group.  The server must be removed from the failover group first.",3);
...
//             if($rows <= 1){
//                 printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Subnet ({$subnet['name']}) has DHCP parameters assigned which need to be removed first",3);
