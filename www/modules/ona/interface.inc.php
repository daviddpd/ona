...
function interface_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_add({$options}) called", 3);
...
EOM
    ...
    if (!$host['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The host specified, {$options['host']}, does not exist!",3);
        $self['error'] = "ERROR => The host specified, {$options['host']}, does not exist!";
        return(array(2, $self['error'] . "\n"));
    }
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host selected: {$options['host']}", 3);

    // Translate IP address to a number
    $orig_ip= $options['ip'];
    $options['ip'] = ip_mangle($options['ip'], 1);
    if ($options['ip'] == -1) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid IP address ({$orig_ip})",3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => IP conflict: That IP address (" . ip_mangle($orig_ip,'dotted') . ") is already in use!",3);
    ...
    if ($status or $rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => IP conflict: That IP address (" . ip_mangle($orig_ip,'dotted') . ") falls within a DHCP address pool!",3);
    ...
    if ($status or $rows != 1 or !$subnet['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => That IP address (" . ip_mangle($orig_ip,'dotted') . ") is not inside a defined subnet!",3);
        $self['error'] = "ERROR => That IP address (" . ip_mangle($orig_ip,'dotted') . ") is not inside a defined subnet!";
        return(array(6, $self['error'] . "\n"));
    }
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Subnet selected: {$subnet['description']}", 3);
    ...
    if ($subnet['ip_mask'] < 4294967294) {
        if ($options['ip'] == $subnet['ip_addr']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => IP address (" . ip_mangle($orig_ip,'dotted') . ") can't be a subnet's base address!{$subnet['ip_addr']}",3);
        ...
        if ($options['ip'] == ((4294967295 - $subnet['ip_mask']) + $subnet['ip_addr']) ) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => IP address (" . ip_mangle($orig_ip,'dotted') . ") can't be a subnet's broadcast address!",3);
    ...
    if ($options['mac']) {
        ...
        if ($options['mac'] == -1) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The MAC address specified ({$orig_mac}) is invalid!",3);
        ...
        if ($options['force'] == 'N') {
            ...
            if ($status or $rows) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => MAC conflict: That MAC address ({$options['mac']}) is already in use on another host!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new interface: $id", 3);
    ...
    if ($options['addptr'] == 'Y') {
        ...
        $ptropts['type'] = 'PTR';
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_add() calling dns_record_add() for new PTR record: {$options['ip']}", 3);
    ...
    if ($options['natip']) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_add() calling nat_add() for new ip: {$options['natip']}", 3);
...
function interface_modify($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_modify({$options}) called", 3);
...
EOM
    ...
    // If a hostname was provided, do a search based on that
        ...
        list($status, $rows, $host) = ona_find_host($options['host']);
        if (!$host['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host not found ({$options['host']})!",3);
        ...
        if ($options['use_primary'] == 'Y') {
            ...
            if ($rows > 1) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Specified host ({$options['host']}) has more than one interface!",3);
    ...
    if (!$interface or !$interface['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Interface not found ({$options['interface']})!",3);
    ...
    if ($options['set_ip']) {
        ...
        if ((is_ipv4($options['set_ip']) && ($options['set_ip'] == $subnet['ip_addr'])) || (!is_ipv4($options['set_ip']) && (!gmp_cmp(gmp_init($options['set_ip']),gmp_init($subnet['ip_addr'])))) ) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => IP address (" . ip_mangle($orig_ip,'dotted') . ") can't be a subnet's base address!",3);
            $self['error'] = "ERROR => IP address (" . ip_mangle($orig_ip,'dotted') . ") can't be a subnet's base address!";
            return(array(8, $self['error'] . "\n"));
        }
        ...
        if ($options['set_mac']) {  // allow null mac addresses (to unset one for example)
            ...
            if ($options['force'] != 'Y') {
                ...
                if (($rows and $record['id'] != $interface['id']) or $rows > 1) {
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => MAC conflict: That MAC address ({$options['set_mac']}) is already in use on another host!",3);
...
function interface_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_del({$options}) called", 3);
...
EOM
    ...
    if ($options['commit'] == 'Y') {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => total shared host records => {$clust_rows}", 3);
        if ($clust_rows > 0) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => There are {$clust_rows} hosts sharing this interface, remove the shares first.",3);
        ...
        if (!isset($options['delete_by_module'])) {
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => total interfaces => {$total_interfaces}", 3);
            if ($total_interfaces == 1) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => You cannot delete the last interface on a host, you must delete the host itself ({$host['fqdn']}).",3);
                $self['error'] = "ERROR => You can not delete the last interface on a host, you must delete the host itself ({$host['fqdn']}).";
                return(array(13, $self['error'] . "\n"));
            }
        }

        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Deleting interface: ID {$interface['id']}", 3);
        ...
        list($status, $rows, $record) = ona_get_interface_record(array('host_id' => $interface['host_id']));
            if ($rows == 0) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "WARNING => Host {$host['fqdn']} has NO remaining interfaces!", 0);
    ...
    if ($interface['nat_interface_id'] > 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_del() calling interface_del() for external NAT ip: {$options['nat_interface_id']}", 3);
...
function interface_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_display({$options}) called", 3);
...
function interface_move($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_move({$options}) called", 3);
...
EOM
    ...
    if (!$old_subnet or !$old_subnet['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Source start address ({$options['start']}) isn't valid!", 3);
    ...
    if ($options['end']) {
        ...
        if (!$old_subnet_end or !$old_subnet_end['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Source end address ({$options['end']}) isn't valid!", 3);
            $self['error'] = "ERROR => Source (end) address specified isn't valid!";
            return(array(3, $self['error'] . "\n"));
        }
        if ($old_subnet_end['id'] != $old_subnet['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Both source addresses ({$options['start']} and {$options['end']}) must be on the same subnet!", 3);
    ...
    // but to make our lives easier we set the "end" = "start";
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Only moving one host source={$options['start']}!", 3);
    ...
    if (!$new_subnet or !$new_subnet['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Destination start address ({$options['new_start']}) isn't valid!", 3);
    ...
    if ($old_subnet['id'] == $new_subnet['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Both the source IP range ({$options['start']}+) and the destination IP range ({$options['new_start']}+) are on the same subnet!", 3);
    ...
    if ($options['new_end']) {
        ...
        if (!$new_subnet_end or !$new_subnet_end['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Destination end address ({$options['new_end']}) isn't valid!", 3);
            $self['error'] = "ERROR => Destination (new_end) address specified isn't valid!";
            return(array(3, $self['error'] . "\n"));
        }
        if ($new_subnet_end['id'] != $new_subnet['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Both destination addresses ({$options['new_start']} and {$options['new_end']}) must be on the same subnet!", 3);
    ...
    // but to make our lives easier we set the "end" = "start";
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Only moving one host destination={$options['new_start']}!", 3);
    ...
    if ($total_to_move == 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => There are no interfaces in the source address block!", 3);
...
//         if ($status or $rows) {
//             printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Source host {$ddns['fqdn']} already has an interface on the destination subnet!",3);
...
//         }
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Asked to move {$total_to_move} interfaces to new range: " . ip_mangle($low_ip, 'dotted') . ' - ' . ip_mangle($high_ip, 'dotted'), 0);
    ...
    foreach (array_keys($to_move) as $i) {
        while ($low_ip <= $high_ip) {
            ...
            if ($rows == 0 and $status == 0) {
                ...
                $pool_interfering = 1;
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Couldn't use the DHCP POOL address: " . ip_mangle($low_ip, 'dotted'), 3);
            }
            $low_ip++;
        }
    }
    ...
    if ($total_assigned != $total_to_move) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The destination IP range doesn't have enough free IP addresses!", 3);
...
function interface_move_host($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_move_host({$options}) called", 3);
...
EOM
    ...
    if (!$interface['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The interface specified, {$options['ip']}, does not exist!",3);
    ...
    if ($primarydns['interface_id'] == $interface['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => This interface is part of the primary DNS name for {$primaryhost['fqdn']}, please assign a new primary DNS.",3);
...
//         return(array(5, $self['error'] . "\n"));
//     }
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Interface selected: {$options['ip']}", 3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_move_host() New host is clustered with this IP, Deleting cluster record", 3);
    if ($rows == 1) {
    ...
    if ($interface['nat_interface_id'] > 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_move_host() Moving interface with NAT IP.", 3);
...
function interface_share($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_share({$options}) called", 3);
...
EOM
    ...
    list($status, $rows, $int_cluster) = db_get_records($onadb, 'interface_clusters', array('host_id' => $host['id'],'interface_id' => $interface['id']), '', 0);
    if ($rows == 1) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => This host is already clustered with that IP ({$host['fqdn']}-{$interface['ip_addr']}).", 3);
...
function interface_share_del($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => interface_share_del({$options['ip']}) called", 3);
...
EOM
    ...
    list($status, $rows, $int_cluster) = db_get_records($onadb, 'interface_clusters', array('host_id' => $host['id'],'interface_id' => $interface['id']), '', 0);
    if ($rows == 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find share ({$host['fqdn']}-{$interface['ip_addr_text']}).", 3);
...
function nat_add($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => nat_add({$options}) called", 3);
...
EOM
    ...
    if ($interface['nat_interface_id'] > 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The interface specified, already has a NAT IP!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => nat_add() calling interface_add() for new ip: {$options['natip']}", 3);
...
function nat_del($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => nat_del({$options}) called", 3);
...
EOM
    ...
    if (!$natinterface['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The NAT interface specified, {$options['natip']}, does not exist!",3);
        $self['error'] = "ERROR => The NAT interface specified, {$options['natip']}, does not exist!";
        return(array(3, $self['error'] . "\n"));
    }
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => NAT Interface selected: {$options['natip']}", 3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => nat_del() calling interface_del() for ip: {$options['natip']}", 3);
