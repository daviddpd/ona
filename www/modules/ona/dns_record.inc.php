...
function dns_record_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_add({$options}) called", 3);
...
*/
    ...
    // Find the domain name piece of $search.
    if (!isset($domain['id'])) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to determine domain name portion of ({$options['name']})!", 3);
        $self['error'] = "ERROR => Unable to determine domain name portion of ({$options['name']})!";
        return(array(3, $self['error'] . "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['name']}) returned: {$domain['fqdn']}", 3);
    ...
    if (!$hostname) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Invalid host name ({$options['name']})!", 3);
    ...
    // Debugging
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Using hostname: {$hostname} Domainname: {$domain['fqdn']}, Domain ID: {$domain['id']}", 3);
    ...
    if ((strpos($domain['fqdn'], "ip6.arpa") or strpos($domain['fqdn'], "in-addr.arpa")) and $options['type'] != 'PTR' and $options['type'] != 'NS') {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Only PTR and NS records should use in-addr.arpa or ip6.arpa domains!", 3);
    ...
    if ($options['view']) {
        ...
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_add() Unable to find DNS view: {$options['view']}",3);
    ...
    if ($options['type'] == 'A' or $options['type'] == 'AAAA') {
        ...
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_add() Unable to find existing IP interface: {$options['ip']}",3);
        ...
        if ($d_status or $d_rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS A record named {$hostname}.{$domain['fqdn']} with IP {$interface['ip_addr_text']} already exists!{$viewmsg}",3);
        ...
        if ($c_rows or $c_status) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS CNAME record named {$hostname}.{$domain['fqdn']} already exists!{$viewmsg}",3);
        ...
        if ($options['addptr'] == 'Y') {
            ...
            if ($rows) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS PTR record already exists for this IP interface!{$viewmsg}",3);
...
*/
    // Process PTR record types
        ...
        list($status, $rows, $interface) = ona_find_interface($options['ip']);
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_add() Unable to find IP interface: {$options['ip']}",3);
        ...
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find DNS A record to point PTR entry to! Check that the IP you chose is associated with the name you chose.{$viewmsg}",3);
            ...
            if (($rows > 1)) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find a SINGLE DNS A record to point PTR entry to! In this case, you are only allowed to do this if there is one A record using this name.{$viewmsg}",3);
...
//         if (!$ptrdomain['id']) {
//             printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find a reverse pointer domain for this IP! Add at least the following DNS domain: {$octets[3]}.in-addr.arpa",3);
...
//         }
        if (!$ptrdomain['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => This operation tried to create a PTR record that is the first in this address space.  You must first create at least the following DNS domain: {$octets[$octcount]}{$arpa}",3);
...
*/
    ...
    // Process CNAME record types
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['pointsto']}) returned: {$domain['fqdn']} for pointsto.", 3);
        ...
        if (!$phostname) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Invalid pointsto host name ({$options['pointsto']})!", 3);
            $self['error'] = "ERROR => Invalid pointsto host name ({$options['pointsto']})!";
            return(array(4, $self['error'] . "\n"));
        }
        // Debugging
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Using 'pointsto' hostname: {$phostname}.{$pdomain['fqdn']}, Domain ID: {$pdomain['id']}", 3);
        ...
        if ($d_status or $d_rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS A record named {$hostname}.{$domain['fqdn']} already exists!{$viewmsg}",3);
        ...
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find DNS A record to point CNAME entry to!{$viewmsg}",3);
    ...
    // this will give you "mydomain.com    IN   NS   server.somedomain.com"
        ...
        list($status, $rows, $domain) = ona_find_domain($options['name'],0);
        if (!$domain['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Invalid domain name ({$options['name']})!", 3);
        ...
        // Find the domain name piece of $search
        list($status, $rows, $pdomain) = ona_find_domain($options['pointsto']);
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['pointsto']}) returned: {$pdomain['fqdn']} for pointsto.", 3);
...
//         }
        ...
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find DNS A record to point NS entry to!{$viewmsg}",3);
        ...
        if ($rows or $status) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS NS record for {$domain['fqdn']} pointing to {$options['pointsto']} already exists!{$viewmsg}",3);
    ...
    // MX is a domain_id or host/domain_id that points to another dns_id A record
        ...
        if (!isset($options['mx_preference']) or ($options['mx_preference'] < 0 or $options['mx_preference'] > 65536)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => You must provide an MX preference value when creating MX records!", 3);
        ...
        if (!$domain['id']) {
            ...
            // Debugging
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Using hostname: {$hostname}.{$domain['fqdn']}, Domain ID: {$domain['id']}", 3);
        }
    ...
    // this will give you "_crap._tcp.mydomain.com    IN   SRV 0 2 80  server.somedomain.com"
        ...
        if (!isset($options['srv_pri']) or ($options['srv_pri'] < 0 or $options['srv_pri'] > 65536)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => You must provide an SRV priority value between 0-65535 when creating SRV records!", 3);
        ...
        if (!isset($options['srv_weight']) or ($options['srv_weight'] < 0 or $options['srv_weight'] > 65536)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => You must provide an SRV weight value between 0-65535 when creating SRV records!", 3);
        ...
        if (!isset($options['srv_port']) or ($options['srv_port'] < 0 or $options['srv_port'] > 65536)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => You must provide an SRV port value between 0-65535 when creating SRV records!", 3);
        ...
        list($status, $rows, $pointsto_record) = ona_get_dns_record(array('name' => $phostname, 'domain_id' => $pdomain['id'], 'type' => 'A','dns_view_id' => $add_viewid));
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find DNS A record to point SRV entry to!{$viewmsg}",3);
        ...
        if ($rows or $status) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS SRV record for {$hostname}.{$domain['fqdn']} pointing to {$options['pointsto']} already exists!{$viewmsg}",3);
    ...
    // Process TXT record types
        ...
        if ($rows or $status) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS TXT record for {$options['name']} with that text value already exists!{$viewmsg}",3);
    ...
    // If it is not a recognized record type, bail out!
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Invalid DNS record type: {$options['type']}!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new dns record: $id", 3);
    ...
    if ($options['addptr'] == 'Y' and $options['type'] == 'A') {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Auto adding a PTR record for {$options['name']}.", 4);
        ...
        if ($status) {
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $output,3);
        }
    }
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $text,0);
    $text .= "\n";
...
function dns_record_modify($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_modify({$options}) called", 3);
...
*/
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_modify() DNS record: {$dns['fqdn']}", 3);
    if ($rows > 1) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Found more than one DNS record for: {$options['name']}",3);
    ...
    if (!$dns['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => DNS record not found ({$options['name']})!",3);
    ...
    if (array_key_exists('set_view',$options)) {
        ...
        list($status, $rows, $dnsview) = ona_get_dns_view_record($viewsearch);
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_modify() Unable to find DNS view: {$options['set_view']}",3);
        ...
        if($dns['dns_view_id'] != $dnsview['id']) {
            ...
            if ($dns['dns_id']) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => You must change the parent DNS A record to the new view.  This record will follow.",3);
    ...
    if ($options['set_ip'] and ($options['set_ip'] != '0.0.0.0')) {
        ...
        if (!$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_modify() Unable to find IP interface: {$options['set_ip']}",3);
        ...
        if ($interface['id'] != $dns['interface_id']) {
            ...
            if ($rows) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => dns_record_modify() This change results in a duplicate child DNS record: PTR {$options['set_ip']}. Delete existing PTR record first.",3);
    ...
    if ($options['set_name']) {
        ...
        // Find the domain name piece of $search
        if (!isset($domain['id'])) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to determine domain name portion of ({$options['set_name']})!", 3);
            $self['error'] = "ERROR => Unable to determine domain name portion of ({$options['set_name']})!";
            return(array(3, $self['error'] . "\n"));
        }
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['set_name']}) returned: {$domain['fqdn']} for new name.", 3);
        ...
        $hostname = sanitize_hostname($hostname);
        if (!$hostname) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid host name ({$options['set_name']})!", 3);
        ...
        if ($dns['type'] == 'A') {
            ...
            if ($rows) {
                if ($tmp['id'] != $dns['id'] or $rows > 1) {
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => There is already an A record with that name and IP address!{$viewmsg}",3);
        ...
        if ($dns['type'] == 'CNAME' or $dns['type'] == 'MX' or $dns['type'] == 'NS' or $dns['type'] == 'SRV') {
            ...
            if ($rows) {
                if ($tmp['id'] != $dns['id'] or $rows > 1) {
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => There is already a {$dns['type']} with that name pointing to that A record!{$viewmsg}",3);
        ...
        if ($dns['type'] == 'CNAME') {
            ...
            if ($status or $rows) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => There is already an A record with that name!{$viewmsg}",3);
    ...
    if (array_key_exists('set_pointsto', $options) and ($options['set_type'] == 'CNAME' or $options['set_type'] == 'MX' or $options['set_type'] == 'NS' or $options['set_type'] == 'SRV')) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['set_pointsto']}) returned: {$domain['fqdn']} for pointsto.", 3);
        ...
        $phostname = sanitize_hostname($phostname);
        if (!$phostname) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Invalid pointsto host name ({$options['set_pointsto']})!", 3);
        ...
        if ($status or !$rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find DNS A record to point {$options['set_type']} entry to!{$viewmsg}",3);
        ...
        if ($c_record['id'] != $dns['id'] and $c_rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Another DNS {$options['set_type']} record exists with the values you've selected!{$viewmsg}",3);
    ...
    if ($options['set_addptr'] == 'Y' and $options['set_type'] == 'A') {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Auto adding a PTR record for {$options['set_name']}.", 0);
        // Run dns_record_add as a PTR type
        ...
        if ($status)
            return(array($status, $output));
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $text);
    }
    ...
    if(count($SET) > 0) {
        ...
        if ($changingint) {
            ...
            if ($SET['interface_id'] != $current_int_id) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG = > dns_record_modify() Updating child interfaces to new interface.", 2);
        ...
        if ($changingview) {
            if ($SET['dns_view_id'] != $current_dns_view_id) {
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG = > dns_record_modify() Updating child DNS records to new dns view.", 2);
    ...
    if($more != '') printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $log_msg, 0);
...
function dns_record_del($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_del({$options}) called", 3);
...
*/
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_del() DNS record: {$options['name']}", 3);
    if (!$dns['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unknown DNS record: {$options['name']} ({$options['type']})",3);
    ...
    if ($options['commit'] == 'Y') {
        ...
        if ($rows) {
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => {$rows} child DNS record(s) DELETED from {$dns['fqdn']}",0);
...
function dns_record_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_display({$options}) called", 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dns_record_del() DNS record: {$record['name']}", 3);
    if (!$record['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unknown DNS record: {$options['name']}",3);
