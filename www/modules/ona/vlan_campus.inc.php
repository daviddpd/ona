...
function vlan_campus_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => vlan_campus_add({$options}) called", 3);
...
EOM
    ...
    if ($status or $rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The vlan campus {$options['name']} already exists!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new VLAN Campus: $id", 3);
...
function vlan_campus_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => vlan_campus_del({$options}) called", 3);
...
EOM
    ...
    if (is_numeric($options['name'])) {
        ...
        if (!$campus['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find VLAN campus using the ID {$options['name']}!",3);
        ...
        list($status, $rows, $campus) = ona_get_vlan_campus_record(array('name' => $options['name']));
        if (!$campus['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find VLAN campus using the name {$options['name']}!",3);
    ...
    if ($rows != 0) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => This VLAN campus ({$campus['name']}) is in use by {$rows} VLAN(s)!",3);
...
function vlan_campus_modify($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => vlan_campus_modify({$options}) called", 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Found VLAN campus: {$campus['name']}", 3);
    ...
    if ($options['set_name']) {
        ...
        if ($status or $rows) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The VLAN campus ({$options['set_name']}) already exists!",3);
...
function vlan_campus_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => vlan_campus_display({$options}) called", 3);
...
EOM
    ...
    if (is_numeric($options['campus'])) {
        ...
        if (!$campus['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find campus using the ID {$options['campus']}!",3);
        ...
        if (!$campus['id']) {
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find campus using the name {$options['campus']}!",3);
            return(array(2, $self['error'] . "\n"));
        }
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Found campus: {$campus['name']}", 3);
