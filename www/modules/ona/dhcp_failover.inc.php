...
function dhcp_failover_group_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_add({$options}) called", 3);
...
EOM
    ...
    if ($options['pri_server']) {
        ...
        if (!$pri_server['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server specified, {$options['pri_server']}, does not exist!",3);
...
/*        list($status, $rows, $pri_server) = ona_get_server_record(array('host_id' => $pri_host['id']));
        ...
        if (!$pri_server['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The host specified, {$pri_host['fqdn']}, is not a server!",3);
    ...
    if ($options['sec_server']) {
        ...
        if (!$sec_server['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server specified, {$options['sec_server']}, does not exist!",3);
...
/*        list($status, $rows, $sec_server) = ona_get_server_record(array('HOST_id' => $sec_host['id']));
        ...
        if (!$sec_server['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The host specified, {$sec_host['fqdn']}, is not a server!",3);
    ...
    if ($pri_server['id'] == $sec_server['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The primary server and the secondary server cannot be the same ({$pri_host['fqdn']}).",3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => A failover group using, {$options['pri_server']} and {$options['sec_server']}, already exists!",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_add(): New failover group id: {$id}", 3);
...
function dhcp_failover_group_del($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_del({$options}) called", 3);
...
EOM
    ...
    if (!$entry['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find a DHCP failover group record using id {$options['id']}!",3);
        $self['error'] = "ERROR => Unable to find a DHCP failover group record using id {$options['id']}!";
        return(array(4, $self['error']. "\n"));
    }

    // Debugging
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => DHCP failover group selected: {$entry['id']}", 3);


    // Display an error if pools are using this zone
    list($status, $rows, $pool) = db_get_record($onadb, 'dhcp_pools', array('id' => $entry['id']));
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => DHCP failover group ({$entry['id']}) can't be deleted, it is in use on 1 or more pools!",3);
...
function dhcp_failover_group_modify($options="") {
    global $conf, $self, $onadb;
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_modify({$options}) called", 3);
...
EOM
    ...
    if (!$failovergroup['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find the DHCP failover group record using {$options['id']}!",3);
    ...
    // Debugging
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_display(): Found id:{$failovergroup['id']}", 3);
    ...
    if (array_key_exists('set_pri_server',$options) and $options['set_pri_server']) {
        ...
        if (!$pri_host['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server specified, {$options['set_pri_server']}, does not exist!",3);
...
//         }
    ...
    if (array_key_exists('set_sec_server',$options) and $options['set_sec_server']) {
        ...
        if (!$sec_host['id']) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The server specified, {$options['set_sec_server']}, does not exist!",3);
...
function dhcp_failover_group_display($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => dhcp_failover_group_display({$options}) called", 3);
