...
function domain_server_add($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_server_add({$options}) called", 3);
...
//     }
    ...
    if (!$domain['id']) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find the domain record using {$options['domain']}!",3);
        $self['error'] = "ERROR => Unable to find the domain record using {$options['domain']}!";
        return(array(4, $self['error']. "\n"));
    }

    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_server_add(): Found domain, {$domain['name']}", 3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Domain {$domain['name']} already assigned to {$options['server']}",3);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_server_add(): New DNS server domain ID: $id", 3);
    ...
    if (!$dnsrows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Auto adding a NS record for {$options['server']}.", 0);
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Found existing NS record for {$options['server']}. Skipping the auto add.", 0);
    }
...
function domain_server_del($options="") {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_server_del({$options}) called", 3);
...
EOM
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => domain_server_del(): Found domain, {$domain['name']}", 3);
    ...
    list($status, $rows, $domainserver) = ona_get_dns_server_domain_record(array('host_id' => $host['id'],'domain_id' => $domain['id']));
    if (!$rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find {$domain['name']} on server {$host['fqdn']}",3);
