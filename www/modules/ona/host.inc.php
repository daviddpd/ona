...
function host_add($options="") {
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_add({$options}) called", 3);
...
EOM
    ...
    if ($rows) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_add() IP conflict: That IP address (" . ip_mangle($orig_ip,'dotted') . ") is already in use!",3);
    ...
    if ($options['location']) {
        ...
        if ($status or !$rows) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The location specified, {$options['location']}, does not exist!", 3);
    ...
    if ($status or $rows != 1 or !$device_type['id']) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The device type specified, {$options['type']}, does not exist!", 3);
        return(array(3, "ERROR => The device type specified, {$options['type']}, does not exist!\n"));
    }
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Device type selected: {$device_type['model_description']} Device ID: {$device_type['id']}", 3);
    ...
    if ($options['security_level'] == -1) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Sanitize security level failed either ({$options['security_level']}) is invalid or is higher than user's level!", 3);
    ...
    if (!isset($domain['id'])) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to determine domain name portion of ({$options['host']})!", 3);
        $self['error'] = "ERROR => Unable to determine domain name portion of ({$options['host']})!";
        return(array(3, $self['error'] . "\n"));
    }

    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ona_find_domain({$options['host']}) returned: {$domain['fqdn']}", 3);
    ...
    if (!$hostname) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Invalid host name ({$options['host']})!", 3);
    ...
    if ($d_status or $d_rows) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The name {$hostname}.{$domain['fqdn']} is already in use, the primary name for a host should be unique!",3);
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new host record: $id", 3);
    ...
    if (!$options['device']) {
        ...
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => ID for new device record: $id", 3);
    } else {
        list($status, $rows, $devid) = ona_find_device($options['device']);
        if (!$rows) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The device specified, {$options['device']}, does not exist!",3);
...
// FIXME: MP this should use the run_module('device_add')!!! when it is ready
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_add() ({$hostname}.{$domain['fqdn']}) calling interface_add() ({$options['ip']})", 3);
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_add() ({$hostname}.{$domain['fqdn']}) calling dns_record_add() ({$options['ip']})", 3);
...
function host_modify($options="") {
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_modify({$options}) called", 3);
...
EOM
    ...
    if ($options['set_type']) {
        ...
        if ($status or $rows != 1 or !$device_type['id']) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The device type specified, {$options['set_type']}, does not exist!",3);
            $self['error'] = "ERROR => The device type specified, {$options['set_type']}, does not exist!";
            return(array(6, $self['error'] . "\n"));
        }
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Device type ID: {$device_type['id']}", 3);
    ...
    if (array_key_exists('set_device', $options)) {
        ...
        if (!$rows) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The device specified, {$options['set_device']}, does not exist!",3);
    ...
    if (array_key_exists('set_location', $options)) {
        if (!$options['set_location'])
            ...
            if (!$rows) {
                printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => The location specified, {$options['set_location']}, does not exist!",3);
...
function host_del($options="") {
    global $conf, $self, $onadb;
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_del({$options}) called", 3);
...
EOM
    ...
    list($status, $rows, $host) = ona_find_host($options['host']);
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_del() Host: {$host['fqdn']} ({$host['id']})", 3);
    if (!$host['id']) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unknown host: {$host['fqdn']}",3);
    ...
    if ($options['commit'] == 'Y') {
        ...
        if ($rows) $serverrow++;
        if ($serverrow > 0) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host ({$host['fqdn']}) cannot be deleted, it is performing duties as a DHCP server!",3);
        ...
        if ($serverrow > 0) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host ({$host['fqdn']}) cannot be deleted, it is performing duties as a DNS server!",3);
        ...
        if ($rows) {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host ({$host['fqdn']}) cannot be deleted, it has config archives!",3);
        ...
        // log deletions
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => {$rows} Shared interface(s) DELETED from {$host['fqdn']}",0);
...
//         }
        ...
        // log deletions
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => {$rows} Message(s) DELETED from {$host['fqdn']}",0);
        ...
        if ($rows == 1) {
            ...
            // log deletions
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Device record DELETED: [{$record['id']}] no remaining hosts using this device",0);
        } else {
            printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Device record NOT DELETED: [{$record['id']}] there are other hosts using this device.",1);
        }
...
function host_display($options="") {
    ...
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => host_display({$options}) called", 3);
...
EOM
    ...
    // Find the host (and domain) record from $options['host']
    list($status, $rows, $host) = ona_find_host($options['host']);
    printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Host: {$host['fqdn']}", 3);
    if (!$host['id']) {
        printmg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unknown host: {$options['host']}",3);
