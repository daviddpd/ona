...
class auth_ldap extends auth_local {
    ...
    function auth_ldap(){
        ...
        if(!function_exists('ldap_connect')) {
            if ($this->cnf['debug'])
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap err: PHP LDAP extension not found.",0);
            $this->success = false;
            return;
        }
    ...
    function checkPass($user,$pass){
        ...
        if($this->cnf['binddn'] && $this->cnf['bindpw']){
            ...
            if(!@ldap_bind($this->con,$this->cnf['binddn'],$this->cnf['bindpw'])){
                if($this->cnf['debug'])
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP bind as superuser: '.htmlspecialchars(ldap_error($this->con)),1);
                return false;
            }
            $this->bound = 2;
            ...
            if(!@ldap_bind($this->con)){
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap: can not bind anonymously",0);
                if($this->cnf['debug'])
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP anonymous bind: '.htmlspecialchars(ldap_error($this->con)),1);
                return false;
            }
        }
        ...
        if(!empty($dn)) {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => auth_ldap: binding with DN: $dn", 5);
            // User/Password bind
            if(!@ldap_bind($this->con,$dn,$pass)){
                if($this->cnf['debug']){
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap: bind with $dn failed", 1);
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP user dn bind: '.htmlspecialchars(ldap_error($this->con)),1);
                }
                return false;
            }
            $this->bound = 1;
            ...
            if(!@ldap_bind($this->con,$dn,$pass)){
                if($this->cnf['debug']){
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap: bind with $dn failed", 1);
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP user bind: '.htmlspecialchars(ldap_error($this->con)),1);
                }
                return false;
            }
            $this->bound = 1;
    ...
    function getUserData($user,$inbind=false) {
        ...
        if($this->cnf['debug']){
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP user search: '.htmlspecialchars(ldap_error($this->con)),1);
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP search at: '.htmlspecialchars($base.' '.$filter),1);
        }
        ...
        if ($this->cnf['grouptree'] && $this->cnf['groupfilter']) {
            ...
            if(!$sr){
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap: Reading group memberships failed",0);
                if($this->cnf['debug']){
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP group search: '.htmlspecialchars(ldap_error($this->con)),1);
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP search at: '.htmlspecialchars($base.' '.$filter),1);
                }
                return false;
            }
            ...
            if(is_array($result)) foreach($result as $grp){
                if(!empty($grp[$this->cnf['groupkey']][0])){
                    if($this->cnf['debug'])
                        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP usergroup: '.htmlspecialchars($grp[$this->cnf['groupkey']][0]),2);
    ...
    function _openLDAP(){
        ...
        if(!$this->con){
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => auth_ldap: couldn't connect to LDAP server",0);
            return false;
        }
        ...
        if($this->cnf['version']){
            if(!@ldap_set_option($this->con, LDAP_OPT_PROTOCOL_VERSION,
                                 $this->cnf['version'])){
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'ERROR => auth_ldap: Setting LDAP Protocol version '.$this->cnf['version'].' failed',0);
                if($this->cnf['debug'])
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP version set: '.htmlspecialchars(ldap_error($this->con)),1);
            }else{
                //use TLS (needs version 3)
                if($this->cnf['starttls']) {
                    if (!@ldap_start_tls($this->con)){
                        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'ERROR => auth_ldap: Starting TLS failed',0);
                        if($this->cnf['debug'])
                            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP TLS set: '.htmlspecialchars(ldap_error($this->con)),1);
                    }
                }
                // needs version 3
                if(isset($this->cnf['referrals'])) {
                    if(!@ldap_set_option($this->con, LDAP_OPT_REFERRALS,
                       $this->cnf['referrals'])){
                        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'ERROR => auth_ldap: Setting LDAP referrals to off failed',0);
                        if($this->cnf['debug'])
                            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP referal set: '.htmlspecialchars(ldap_error($this->con)),1);
                    }
                }
            }
        }
        ...
        if($this->cnf['deref']){
            if(!@ldap_set_option($this->con, LDAP_OPT_DEREF, $this->cnf['deref'])){
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'ERROR => auth_ldap: Setting LDAP Deref mode '.$this->cnf['deref'].' failed',0);
                if($this->cnf['debug'])
                    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . 'DEBUG => auth_ldap: LDAP deref set: '.htmlspecialchars(ldap_error($this->con)),1);
            }
        }
