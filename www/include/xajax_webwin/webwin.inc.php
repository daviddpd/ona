...
function window_submit($window_name, $form='', $function='') {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => webwin_submit() Window: {$window_name} Function: {$function} Form: {$form}", 1);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "NOTICE => webwin_submit() invalid function called! Window: {$window_name} Function: {$function}", 0);
    return($response->getXML());
}
