...
//FIXME: add some error logging in the web gui if we get failures here
        ...
        if (class_exists($auth_class)) {
            ...
            if ($auth->success == false) {
                ...
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Failure loading auth module: {$conf['authtype']}.", 0);
            }
        } else {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Unable to find auth class: {$auth_class}.", 0);
        }
    } else {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Auth module {$authtype} not in path: ".ONA_AUTH, 0);
    }
    return($auth);
}
...
function get_authentication($login_name='', $login_password='') {
    ...
    if (!preg_match('/^[A-Za-z0-9.\-_]+$/', $login_name)) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Login failure for {$login_name}: Bad username format", 0);
        return(array(1, $js));
    }
    ...
    if ($login_name == 'guest') {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Guest user login, forcing local auth.",1);
    ...
    if ($auth->founduser === false) {
        ...
        if ($conf['authtype'] != 'local') {
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Unable to find user via auth_{$conf['authtype']}, falling back to local auth_local.",1);
            $auth = load_auth_class('local');
            $authresult = $auth->checkPass($login_name,$login_password);
            if ($auth->founduser === false) {
                $js = "el('loginmsg').innerHTML = 'Unknown user';";
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Login failure for {$login_name}: Unknown user", 0);
                return(array(1, $js));
            }
    ...
    if ($authresult === false) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => Login failure for {$login_name} using authtype {$conf['authtype']}: Password incorrect", 0);
        return(array(1, $js));
    }

    // If the password is good.. return success.
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Authentication Successful for {$login_name} using authtype: {$conf['authtype']}", 1);
    return(array(0, $js));
}
...
function get_perms($login_name='') {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Authorization Starting for {$login_name}", 1);

    // get user information and groups from the previously populated auth class
    $userinfo = $auth->getUserData($login_name);
    if ($userinfo === false) printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Failed to get user information for user: {$login_name}", 0);
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "INFO => Loaded permissions for " . $login_name, 2);
    return true;
...
function auth_cryptPassword($clear,$method='',$salt=null){
    ...
    switch(strtolower($method)){
        ...
        case 'kmd5':
            ...
            return $hash2;
        default:
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "Unsupported crypt method $method",0);
    }
}
