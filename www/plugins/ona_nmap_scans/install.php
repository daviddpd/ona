    ...
    foreach ($pmodules as $modname => $attributes) {
        if (!array_key_exists($modname,$self['cache']['modules'])) {
            ...
            if ($status) {
                ...
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Plugin install for {$plugin_name} created new DCM module {$modname}.",2);
...
//print_r($statement);
    ...
    if ($has_trans) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Starting transaction", 2);
        $onadb->StartTrans();
    }


    // Run the SQL
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => Installing {$modname} plugin SQL statements.", 4);
    $i = 0;
