...
$window['title'] = "Custom Attribute Type Administration";

// Load some html into $window['html']
$form_id = "{$window_name}_filter_form";
$tab = 'custom_attribute_type';
    ...
    <table width="100%" cellspacing="0" border="0" cellpadding="0" >
        <tr>
            <td id="{$form_id}_{$tab}_tab" nowrap="true" class="table-tab-active">
                Custom Attribute Types <span id="{$form_id}_{$tab}_count"></span>
            </td>
...
function ws_display_list($window_name, $form) {
    ...
    <table cellspacing="0" border="0" cellpadding="0" width="100%" class="list-box">
        ...
        <!-- Table Header -->
        <tr>
            <td class="list-header" align="center" style="{$style['borderR']};">Type Name</td>
...
EOL;
    ...
    foreach ($records as $record) {
        ...
        <tr onMouseOver="this.className='row-highlight'" onMouseOut="this.className='row-normal'">
            ...
            <td class="list-row">
                <a title="Edit type. id: {$record['id']}"
                   class="act"
                   onClick="xajax_window_submit('app_custom_attribute_type_edit', '{$record['id']}', 'editor');"
                >{$record['name']}</a>&nbsp;
            </td>
            ...
            <td align="right" class="list-row" nowrap="true">
                ...
                <a title="Delete type. id: {$record['id']}"
                    class="act"
                    onClick="var doit=confirm('Are you sure you want to delete this custom attribute type?');
                            if (doit == true)
...
EOL;
    ...
    <div class="act-box" style="padding: 2px 4px; border-top: 1px solid {$color['border']}; border-bottom: 1px solid {$color['border']};">
        ...
        <a title="New type"
            class="act"
            onClick="xajax_window_submit('app_custom_attribute_type_edit', ' ', 'editor');"
        ><img src="{$images}/silk/page_add.png" border="0"></a>&nbsp;

        <a title="New type"
            class="act"
            onClick="xajax_window_submit('app_custom_attribute_type_edit', ' ', 'editor');"
        >Add custom attribute type</a>&nbsp;
    </div>
EOL;
...
function ws_delete($window_name, $form='') {
    ...
    list($status, $rows, $type) = db_get_record($onadb,
                                                'custom_attribute_types',
                                                array('id' => $form));
    if ($status or !$rows) {
    ...
    list($status, $rows, $customattributes) = db_get_records($onadb,
                                                'custom_attributes',
                                                array('custom_attribute_type_id' => $form),
                                                '',
                                                0);

    // Check that there are no parent records using this type
    if ($rows > 0) {
        $js .= "alert('Delete failed: There are {$rows} custom attributes using this type.');";
    }
    else {
        // Delete the record
