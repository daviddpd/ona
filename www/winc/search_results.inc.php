...
function quick_search($q) {
    ...
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => quick_search({$q}) called", 3);
    ...
    $mac = mac_mangle($q, 1);
    if ($mac != -1) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => quick_search() Looks like a MAC, Returning mac = {$q}" ,3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => quick_search() returning host match (ID={$record['host_id']})", 3);
    ...
    if ($rows) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => quick_search() returning subnet match (ID={$record['id']})", 3);
        return( array('subnets', array('subnet_id' => $record['id']) ) );
    }

    // Well, I guess we'll assume $q is a hostname/alias search
    printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => quick_search() found no subnet or host match. Returning hostname = {$q}" ,3);
