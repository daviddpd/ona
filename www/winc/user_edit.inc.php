...
function ws_save($window_name, $form='') {
    ...
    if (!$form['id']) {
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "NOTICE => Added new user: {$form['username']} client url: {$_SESSION['auth']['client']['url']}", 0);
    }
    ...
    // Update an existing record?
        ...
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "NOTICE => Updated user: {$form['username']} client url: {$_SESSION['auth']['client']['url']}", 0);
    }
    
    // If the module returned an error code display a popup warning
    if ($status) {
        printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "ERROR => User add/edit failed! {$self['error']}", 0);
