...
$window['title'] = "System Config Administration";
    ...
    <table width="100%" cellspacing="0" border="0" cellpadding="0" >
        <tr>
            <td id="{$form_id}_{$tab}_tab" nowrap="true" class="table-tab-active">
                System Config <span id="{$form_id}_{$tab}_count"></span>
            </td>
...
// This function displays a list (all?)
function ws_display_list($window_name, $form) {
    ...
    <table cellspacing="0" border="0" cellpadding="0" width="100%" class="list-box">
        ...
        <tr>
            ...
            <td class="list-header" align="center" style="{$style['borderR']};">Value</td>
            <td class="list-header" align="center">&nbsp;</td>
        </tr>

EOL;

    $where = 'name LIKE '. $onadb->qstr('%');
...
EOL;
    ...
    if ($record['deleteable'] > 0) {
        $html .= <<<EOL
                <a title="Delete entry."
                    class="act"
                    onClick="var doit=confirm('Are you sure you want to delete this entry?');
                            if (doit == true)
                                xajax_window_submit('{$window_name}', '{$record['name']}', 'delete');"
                ><img src="{$images}/silk/delete.png" border="0"></a>&nbsp;
EOL;
    } else {
        $html .= <<<EOL
                <img src="{$images}/silk/lock.png" border="0" title="Locked system record, you can not delete it.">&nbsp;
EOL;
    }
...
function ws_delete($window_name, $form='') {
    ...
    list($status, $rows, $syscnf) = db_get_record($onadb,
                                        'sys_config',
                                        array('name' => $form));
    if ($status or !$rows) {
