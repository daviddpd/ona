    ...
    <table width="100%" cellspacing="0" border="0" cellpadding="0" >
        <tr>
            <td id="{$form_id}_{$tab}_tab" nowrap="true" class="table-tab-active">
                Configuration Types <span id="{$form_id}_{$tab}_count"></span>
            </td>
...
function ws_display_list($window_name, $form) {
    ...
    <table cellspacing="0" border="0" cellpadding="0" width="100%" class="list-box">
        ...
        <!-- Table Header -->
        <tr>
            <td class="list-header" align="center" style="{$style['borderR']};">Conf Type</td>
...
EOL;
    ...
    foreach ($records as $record) {
        ...
        list ($status, $usage_rows, $tmp) = db_get_records($onadb, 'configurations', "configuration_type_id = {$record['id']}", '', 0);
        ...
        <tr onMouseOver="this.className='row-highlight'" onMouseOut="this.className='row-normal'">
            ...
            <td class="list-row">
                <a title="Edit config type. id: {$record['id']}"
                   class="act"
                   onClick="xajax_window_submit('app_config_type_edit', '{$record['id']}', 'editor');"
                >{$record['name']}</a>&nbsp;
            </td>
            ...
            <td align="right" class="list-row" nowrap="true">
                ...
                <a title="Delete config type. id: {$record['id']}"
                    class="act"
                    onClick="var doit=confirm('Are you sure you want to delete this configuration type?');
                            if (doit == true)
...
EOL;
    ...
    <div class="act-box" style="padding: 2px 4px; border-top: 1px solid {$color['border']}; border-bottom: 1px solid {$color['border']};">
        <!-- ADD LINK -->
        <a title="New config type"
            class="act"
            onClick="xajax_window_submit('app_config_type_edit', ' ', 'editor');"
...
function ws_delete($window_name, $form='') {
    ...
    list($status, $rows, $conftype) = db_get_record($onadb,
                                                'configuration_types',
                                                array('id' => $form));
    if ($status or !$rows) {
        $response->addScript("alert('Delete failed: Configuration type id {$form} does not exist');");
        return($response->getXML());
    }

    // Get a list of device models that use this manufacturer
    list($status, $rows, $configs) = db_get_records($onadb,
                                                'configurations',
                                                array('configuration_type_id' => $form),
                                                '',
                                                0);

    // Check that there are no parent records using this type
    if ($rows > 0) {
        $js .= "alert('Delete failed: There are {$rows} configurations using this configuration type.');";
    }
    else {
        // Delete the record
        list($status, $rows) = db_delete_records($onadb,
                                    'configuration_types',
                                    array('id' => $conftype['id']));
