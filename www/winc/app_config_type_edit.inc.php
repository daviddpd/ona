...
//
// Description:
//     Displays a form for creating/editing types.
        ...
        <tr>
            ...
            <td class="padding" align="right" width="100%">
                ...
                <input class="edit" type="button"
                    ...
                    value="Save"
                    accesskey=" "
                    onClick="xajax_window_submit('{$window_name}', xajax.getFormValues('config_type_edit_form'), 'save');"
                >
            </td>
        </tr>
...
//
// Description:
//     Creates/updates a type with the info from the submitted form.
//////////////////////////////////////////////////////////////////////////////
function ws_save($window_name, $form='') {
    ...
    $form['config_type_name'] = trim($form['config_type_name']);
    ...
    list($status, $rows, $test) = ona_get_record(array('name' => $form['config_type_name']), 'configuration_types');
    if ($rows) {
    ...
    if (is_numeric($form['id'])) {
        ...
        list($status, $rows, $original_type) = ona_get_record(array('id' => $form['id']), 'configuration_types');

        if ($form['config_type_name'] !== $original_type['name']) {
            list($status, $rows) = db_update_record(
                                         $onadb,
                                         'configuration_types',
                                         array('id' => $form['id']),
                                         array('name' => $form['config_type_name'])
                                     );
            if ($status or !$rows) {
                ...
                list($status, $rows, $new_type) = ona_get_record(array('id' => $form['id']), 'configuration_types');

                // Return the success notice
                $self['error'] = "INFO => Configuration type UPDATED:{$new_type['id']}: {$new_type['name']}";
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $self['error'], 0);
                $log_msg = "INFO => Configuration type UPDATED:{$new_type['id']}: name[{$original_type['name']}=>{$new_type['name']}]";
    ...
    // If you get nothing in $form, create a new record
        ...
        $id = ona_get_next_id('configuration_types');
        ...
        if (!$id) {
            ...
            printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . "DEBUG => id for new config type record: $id", 3);
            list($status, $rows) = db_insert_record($onadb,
                                        "configuration_types",
                                        array('id' => $id,
                                        'name' => trim($form['config_type_name'])));

            if ($status or !$rows) {
                $self['error'] = "ERROR => config_type_edit add ws_save() failed: " . $self['error'];
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $self['error'], 0);
            }
            else {
                $self['error'] = "INFO => Configuration type ADDED: {$form['config_type_name']} ";
                printmsg( pstr(__FILE__,__LINE__,__FUNCTION__) . $self['error'], 0);
            }
        }
    }
    ...
    if ($status or !$rows) {
        ...
        $js .= "xajax_window_submit('app_config_type_list', xajax.getFormValues('app_config_type_list_filter_form'), 'display_list');";
    }
